...
**Status:** Implemented and deployed on Goldsky. A The Graph subgraph that indexes Ponti's on-chain events on Base Sepolia; the app reads groups and expense history from a GraphQL API instead of scanning `eth_getLogs` over the full chain history on every load.
**Known limitation:** runs on a free indexing tier, which can lag or stall; the app degrades honestly (balances stay live via direct contract reads).
...
Decouple read cost from chain-block growth. `fetchMyGroups` and `fetchExpenseHistory` previously scanned `getLogs` over `[FACTORY_DEPLOY_BLOCK, head]` on every load; that range grows with the chain and free RPCs throttle it (Alchemy free caps getLogs at 10 blocks, drpc free times out). The subgraph ingests each event once as it is mined and serves cheap GraphQL queries whose cost scales with results, not block range.

## Scope

What the subgraph serves (replaced the `getLogs` scans):
...
- `fetchMyGroups` and `fetchExpenseHistory` query the subgraph over GraphQL with a minimal `fetch` (no Apollo or other client dependency).
- `reconstructExpenses` was removed (the subgraph does the fold).
- `interpretBalance` stays; `fetchBalance` stays a direct `readContract`.
- The subgraph query URL lives in `config.ts` via the `VITE_SUBGRAPH_URL` env var.
- The chunked `getLogs` / `collectInWindows` read path is retired for these reads (writes still go through the AA stack unchanged).

## Deploy

- Built and deployed with `graph-cli` (`graph deploy`). A `Makefile` recipe wraps codegen/build/deploy.
- The deployed subgraph exposes a query URL consumed by the app via `VITE_SUBGRAPH_URL`.
...
- GraphQL queries return the known test group (counterparty `0xB5EA...`) for the test smart account, with expenses folded correctly (edited/deleted reflected), plus settlements.
- In the app: groups list and expense history load via GraphQL (no `getLogs`); `balance` loads via `readContract`; full E2E (create / add / edit / delete / settle) reflects in the subgraph within a block or two.
